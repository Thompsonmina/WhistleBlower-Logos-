...
/// Watch the Logos Delivery topic and anchor new CIDs in batches.
///
/// Startup sequence:
/// 1. Seed dedup set from on-chain registry.
/// 2. Wait for nwaku REST to be healthy, subscribe to the configured topic.
/// 3. Poll loop: drain → push to buffer → check should_flush → submit on-chain.
pub async fn run(cfg: Config) -> Result<()> {
    ...
    // Store-protocol catch-up: pull whatever nwaku stored in the lookback window
    // so messages broadcast while the anchor was down still get anchored.  The
    // dedup set (seeded from on-chain) filters out everything we've already done.
