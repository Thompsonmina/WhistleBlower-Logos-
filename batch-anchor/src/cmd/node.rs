//! Wraps `docker compose` for the local nwaku node.
//!
//! All actions delegate to `docker compose -f <compose_file>`, so the command's
//! semantics are inherited from docker-compose: `up -d` is idempotent (no-op if
//! the container is already running), `down` removes the container but keeps
//! the SQLite store volume mounted in `./store`.
...
/// Spawn `docker compose -f <compose_file> <args...>`, inheriting stdio so the
/// user sees docker's output directly (progress bars, container IDs, log tail).
